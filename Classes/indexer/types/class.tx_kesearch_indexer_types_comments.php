...
class tx_kesearch_indexer_types_comments extends tx_kesearch_indexer_types
{
    ...
    public function startIndexing()
    {
        ...
        $where .= ' AND external_prefix IN ("'
            . implode(
                '","',
                TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->indexerConfig['commenttypes'])
            )
            . '")';
        ...
        if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
            ...
            while (($comment = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                ...
                $this->pObj->storeInIndex(
                    ...
                    $abstract,                      // abstract
                    -1,
                    // language uid is -1 because comments is not able to use multiple languages
                    0,                                // starttime
                    0,                                // endtime
                    '',                                // fe_group
                    false,                          // debug only?

 ...
 */
class Templavoila extends IndexerBase
{
    public $pids = 0;
    ...
    public function __construct($pObj)
    {
        ...
        $this->templavoilaIsLoaded = ExtensionManagementUtility::isLoaded('templavoila');
    ...
    public function getPageContent($uid)
    {
        ...
        $flex = GeneralUtility::xml2array($flex);
        ...
        $tvPaths = GeneralUtility::trimExplode(',', $tvPaths);

        foreach ($tvPaths as $tvPath) {
            $contentElementUids[] = $flex['data']['sDEF']['lDEF'][$tvPath]['vDEF'];
        }

        $contentElementUids = GeneralUtility::uniqueList(implode(',', $contentElementUids));

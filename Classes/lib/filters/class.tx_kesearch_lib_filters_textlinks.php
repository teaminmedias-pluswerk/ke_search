...
class tx_kesearch_lib_filters_textlinks
{
    ...
    public $countActiveOptions = 0;
    public $contentOfHiddenFields = array();
    public $maxAllowedNormalOptions = 0;
    public $contentOfActiveOptions = array();
    public $contentOfNormalOptions = array();
    ...
    public function init(tx_kesearch_lib $pObj)
    {
        ...
        // set template array
        $this->templateArray['filter'] = $this->cObj->getSubpart(
            $this->pObj->templateCode,
            '###SUB_FILTER_TEXTLINKS###'
        );
        $this->templateArray['options'] = $this->cObj->getSubpart(
            $this->pObj->templateCode,
    ...
    public function renderTextlinks($filterUid, $optionsOfSearchresult, $lib)
    {
        ...
        if ($this->countActiveOptions) {
            ...
            $linkTextMore = $this->pObj->pi_getLL(
                'linktext_more_' . $filterUid,
                $this->pObj->pi_getLL('linktext_more')
            );
    ...
    public function getOptionsOfFilter($filter, $additionalOptionValues)
    {
        if (is_array($filter) && count($filter) && is_array($additionalOptionValues)) {
            ...
            \TYPO3\CMS\Core\Utility\ArrayUtility::mergeRecursiveWithOverrule(
                $optionsOfCurrentFilter,
                (array)$additionalOptionValues
            );

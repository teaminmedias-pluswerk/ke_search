...
class tx_kesearch_pi3 extends tx_kesearch_lib
{
    ...
    public $scriptRelPath = 'pi3/class.tx_kesearch_pi3.php';
    ...
    public function main($content, $conf)
    {
        ...
        $template['multihidden'] = $this->cObj->getSubpart(
            $template['multiselect'],
            '###SUB_FILTER_MULTISELECT_HIDDEN###'
        );
        $template['multifilter'] = $this->cObj->getSubpart(
            $template['multiselect'],
            '###SUB_FILTER_MULTISELECT_FILTER###'
        );
        $template['multioption'] = $this->cObj->getSubpart(
        ...
        if (is_array($this->piVars['filter']) && count($this->piVars['filter'])) {
            ...
            foreach ($this->piVars['filter'] as $filterKey => $filterValue) {
                ...
                foreach ($this->piVars['filter'][$filterKey] as $optionKey => $option) {
                    $hidden .= $this->cObj->substituteMarker(
                        $template['multihidden'],
                        '###NAME###',
                        'tx_kesearch_pi1[filter][' . $filterKey . '][' . $optionKey . ']'
                    );
